...
function openAppointmentModal() {
    ...
    setValue("appointmentDate", new Date().toISOString().slice(0, 10));
...
function selectCalendarDate(date) {
    ...
    toast(count ? `${count} rendez-vous le ${formatDate(date)}` : `Aucun rendez-vous le ${formatDate(date)}`);
}
...
function renderStatistics() {
    ...
    setText("statsAverage", money(state.invoices.length ? revenue / state.invoices.length : 0));
...
function selectPlan(plan) {
    ...
    if (!confirm(`Activer ${PLANS[plan].name} en mode présentation locale ?`)) return;
...
function hasFeature(feature) {
    ...
    return Boolean(PLANS[state.plan]?.features?.[feature]);
}

function updateSubscriptionPage() {
    setText("currentPlan", PLANS[state.plan]?.name || "GRATUIT");
}

function updatePlanBadge() {
    setText("planBadge", state.ownerUnlocked ? "ACCÈS COMPLET" : PLANS[state.plan]?.name || "GRATUIT");
}
...
function updateSalonHeader() {
    setText("salonHeader", state.settings.salon || "Gestion professionnelle");
    setText("invoiceSalon", state.settings.salon || APP_NAME);
    setText("invoiceOwner", state.settings.owner || "Gestion professionnelle");
}
...
function calculateRevenue() {
    return state.invoices.reduce((s, i) => s + Number(i.total || 0), 0);
}
...
function invoiceNumber() {
    ...
    return `CB-${dateStr}-${String(state.invoices.length + 1).padStart(4, "0")}`;
}
